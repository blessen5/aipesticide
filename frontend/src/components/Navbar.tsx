...
export const Navbar: React.FC<NavbarProps> = ({ currentRole = 'FARMER', onLogout, onRoleChange }) => {
  ...
  const primaryNavItems = [
    ...
    { path: '/scan', label: 'AI Scan', icon: Scan, badge: 'AI' },
  ...
  return (
    <>
      ...
      <div className="h-[2px] w-full bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-600 fixed top-0 left-0 right-0 z-50" />

      {/* Main Spacious Header */}
      <header className="sticky top-0 z-40 bg-slate-950/85 backdrop-blur-2xl border-b border-slate-800/80 shadow-xl shadow-black/20">
          ...
          <div className="flex items-center justify-between h-20 gap-8">
            ...
            <Link to="/" className="flex items-center space-x-3.5 group flex-shrink-0">
              <div className="relative">
                <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-emerald-500 via-emerald-400 to-teal-300 flex items-center justify-center shadow-lg shadow-emerald-500/25 group-hover:shadow-emerald-500/40 group-hover:scale-105 transition-all duration-300">
            ...
            {/* ──────── 2. Desktop Navigation Center (Spacious) ──────── */}
            <nav className="hidden lg:flex items-center space-x-3 xl:space-x-4">
              {primaryNavItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`relative flex items-center space-x-2.5 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                      isActive
                  ...
                  >
                    ...
                    {item.badge && (
                      <span className="bg-emerald-500/90 text-slate-950 text-[9px] font-extrabold px-1.5 py-0.5 rounded-full tracking-wide">
                        {item.badge}
                      </span>
                    )}
                    {isActive && (
                      <span className="absolute bottom-0 left-4 right-4 h-[2px] bg-gradient-to-r from-emerald-500 to-teal-400 rounded-full" />
                    )}
                  </Link>
                );
              })}
              ...
              <div className="relative" ref={moreDropdownRef}>
                ...
                {/* Dropdown Menu */}
                {moreDropdownOpen && (
                  <div className="absolute top-full left-0 mt-3 w-84 rounded-2xl bg-slate-900/95 backdrop-blur-2xl border border-slate-800 shadow-2xl p-2.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-800/80 mb-1.5">
            ...
            {/* ──────── 3. Right Action Controls (Spacious) ──────── */}
            <div className="flex items-center space-x-3.5 sm:space-x-4">
              
              {/* Telemetry Status Pill */}
              <div 
                className={`hidden sm:flex items-center space-x-2.5 px-3.5 py-2 rounded-full text-xs font-medium border backdrop-blur-md transition-all ${
                  healthStatus === 'ONLINE'
              ...
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="flex items-center space-x-2.5 px-3 py-2 rounded-xl bg-slate-900/90 border border-slate-800 hover:border-slate-700 text-xs font-medium text-slate-200 transition shadow-sm"
                ...
                {/* User Menu Dropdown */}
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-3 w-60 rounded-2xl bg-slate-900/95 backdrop-blur-2xl border border-slate-800 shadow-2xl p-2.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
                      ...
                      {onLogout && (
                        <div className="pt-1.5 mt-1.5 border-t border-slate-800">
                          <button
                            ...
                            className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs text-rose-400 hover:bg-rose-950/30 transition"
                          >
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden fixed inset-x-0 top-20 bottom-0 z-50 bg-slate-950/95 backdrop-blur-2xl border-b border-slate-800 overflow-y-auto px-5 py-6 space-y-6 animate-in slide-in-from-top-4 duration-200">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                ...
                <Link
                  to="/demo"
                  className={`flex flex-col p-3.5 rounded-2xl border transition-all col-span-2 ${
                    isDemo
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-xs">Interactive Demo Hub</span>
                    </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
              ...
              {onLogout && (
                <button
                  onClick={onLogout}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-rose-950/40 border border-rose-800/40 text-rose-300 text-xs font-semibold hover:bg-rose-900/40 transition"
                >
                  <LogOut className="w-3.5 h-3.5" /> Sign Out
                </button>
              )}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/90 backdrop-blur-xl border-t border-slate-800/80 px-4 py-2 flex items-center justify-around shadow-2xl">
        <Link
          to="/"
