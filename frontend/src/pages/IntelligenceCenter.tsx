...
export const IntelligenceCenter: React.FC = () => {
  ...
  const loadWeather = React.useCallback(async () => {
    setLoading(true);
    try {
      const u = `https://api.open-meteo.com/v1/forecast?latitude=${FARM.lat}&longitude=${FARM.lon}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,wind_gusts_10m&hourly=precipitation_probability,precipitation&forecast_days=1&timezone=auto`;
      const r = await fetch(u); const d = await r.json();
      const rain6h = (d.hourly?.precipitation ?? []).slice(0,6).reduce((a:number,b:number)=>a+Number(b||0),0);
      const rainProbability = Math.max(...(d.hourly?.precipitation_probability ?? []).slice(0,6).map((x:number)=>Number(x||0)),0);
      setWeather({temperature:d.current.temperature_2m,humidity:d.current.relative_humidity_2m,rainProbability,rain6h,wind:d.current.wind_speed_10m,gust:d.current.wind_gusts_10m});
  ...
  return <div className="space-y-6 animate-in fade-in duration-500">
    <div className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-gradient-to-br from-emerald-950/80 via-zinc-950 to-zinc-950 p-6 shadow-2xl">
      ...
      <div className="relative flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button onClick={loadWeather} className="inline-flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900/80 px-4 py-3 text-sm font-bold hover:border-emerald-500/50"><RefreshCw size={16} className={loading?'animate-spin':''}/> Refresh weather</button>
      </div>
    </div>
    ...
    {g && <div className={`rounded-2xl border p-5 ${g.tone==='emerald'?'border-emerald-500/30 bg-emerald-500/10':'border-amber-500/30 bg-amber-500/10'}`}><div className="flex items-start gap-4"><ShieldCheck className="mt-1 shrink-0"/><div><div className="font-black tracking-wide">{g.label}</div><p className="mt-1 text-sm text-zinc-300">{g.reason}</p></div></div></div>}

    <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
      <section className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900/70"><div className="flex items-center justify-between border-b border-zinc-800 p-4"><div><h2 className="font-black">Field intelligence map</h2><p className="text-xs text-zinc-500">Field polygon and scouting observations — not a road map</p></div><MapPinned className="text-emerald-400" size={19}/></div><div className="h-[430px]"><MapContainer center={[FARM.lat,FARM.lon]} zoom={16} scrollWheelZoom className="h-full w-full"><TileLayer attribution="© Esri" url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"/><Polygon positions={FIELD} pathOptions={{color:'#34d399',fillColor:'#34d399',fillOpacity:.18,weight:3}}/><CircleMarker center={[10.019,76.306]} radius={10} pathOptions={{color:'#f59e0b',fillColor:'#f59e0b',fillOpacity:.8}}><Popup>Zone A · Monitor</Popup></CircleMarker><CircleMarker center={[10.018,76.308]} radius={10} pathOptions={{color:'#ef4444',fillColor:'#ef4444',fillOpacity:.8}}><Popup>Zone B · High attention</Popup></CircleMarker><CircleMarker center={[10.017,76.304]} radius={9} pathOptions={{color:'#22c55e',fillColor:'#22c55e',fillOpacity:.8}}><Popup>Zone C · Healthy</Popup></CircleMarker></MapContainer></div></section>
